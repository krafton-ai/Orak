...
Goal: Analyze last action's outcome, learn, critique, extract facts, detect redundancy, using both text states and visual screenshots.
Core Rules Reminder:
...
4. `PreviousGameState` (obj): `text_state` & `screenshot` (before `LastAction`). (Screenshot always provided)
5. `CurrentGameState` (obj): `text_state` & `screenshot` (after `LastAction`). (Screenshot always provided)
6. `RelevantMemoryEntries`: List[str] (Opt): Factual knowledge relevant to current context.

# TASKS (Be Concise)
1. Action Eval:
   - Success/fail vs. subtask/intent (consider `RelevantMemoryEntries` for context)? Expected vs. actual? Action type apt for state (text/visuals)?
   - Analyze `text_state` changes (state, map, dialog) between `PreviousGameState` & `CurrentGameState`.
   - Compare `PreviousGameState.screenshot` & `CurrentGameState.screenshot`.
2. Critique:
   - Alternatives (informed by text/visuals & `RelevantMemoryEntries`)? Key factors (text/visual/memory insights)?
   - Meta-learn (e.g., combined text/visual/memory cue interpretation, map/state handling, memory utilization)?
   - Followed strategy (aligned with memory & text/visual insights)?
3. Env Summary:
   - Brief `CurrentGameState.text_state` (state, location).
   - New info/entities/obstacles from `text_state` changes (note if conflicts/confirms `RelevantMemoryEntries`)?
   - New relevant visual info from `CurrentGameState.screenshot`?
   - Impact of text/visual changes (contextualized by `RelevantMemoryEntries`)?
4. Extract Facts (`NewFacts`):
   - Verifiable facts from outcome (text state changes, dialog, map reveals). **Cross-reference with `RelevantMemoryEntries` (avoid redundancy, note updates/contradictions).**
   - Include visually verifiable facts from screenshot comparison if new & not fully text-captured (e.g., "Fact: Player faces North (visual)", "Fact: Enemy health bar halved (visual)").
   - Format: "Fact: [S] [P] [O/Details] @ [Loc/Context] (Source: text/visual)". No speculation.
5. Infer Goal: Agent's current goal (all text/visual info & `RelevantMemoryEntries`)? Sensible?
6. Goal Adjust (If Critical): Major change for new text/visual info or blockage (**contextualize significance with `RelevantMemoryEntries`**)? Justify briefly.
...
"Outcome": "(Brief actual outcome, noting key text_state changes and significant visual_state changes)",
"Success": true | false | null
}},
"Env": {{
"Summary": "(Brief summary, e.g., 'Field state, Viridian Forest near sign, player facing sign (visual)')",
"NewInfo": "(Key gained info/changes from text, e.g., 'Sign: TRAINER TIPS!'. Add visual observations if distinct, e.g., 'New item icon appeared on screen (visual)')",
"Entities": ["(e.g., 'SIGN_FOREST_1 (text)')", "(NPC_OAK (text, visual confirmation))"],
"SelectedOption": "(Optional: Option name highlighted by the '▶' cursor, e.g. [option_name] when the line shows "▶[option_name]")"
"Obstacles": ["(e.g., 'Tree @ (X,Y) (text)')"]
}},
"Memory": {{
"Interacted": [ /* {{Name, Content, Map, Loc, Source: text/visual}} */ ],
"Warps": [ /* {{SrcMap, SrcCoord, TgtMap, TgtCoord, Source: text}} */ ]
}},
"Critique": {{
"Factors": "(Brief success/failure factors, considering text and visual evidence)",
...
"MetaNote": "(Concise learning, e.g., 'Visual confirmation of text state is useful')",
"Redundancy": {{
...
"NewFacts": [
"(e.g., 'Fact: SIGN_FOREST_1 at (X,Y) in ViridianForest reads 'TRAINER TIPS!' (Source: text)')",
"(e.g., 'Fact: Player is now at (X+1, Y) (Source: text)')",
"(e.g., 'Fact: A '!' speech bubble appeared over NPC_RIVAL after interaction (Source: visual)')"
]
}}
"""
)
