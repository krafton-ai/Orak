PROMPT = (
    ...
    f"- All actions must be based solely on the on-screen dialogue. The on-screen dialogue is defined as the very last entry in the \"Current State (text)\"'s [Recent Conversations], which is marked as [**The Conversation Currently on Screen**], shown in the \"Current Screen (image screenshot)\" as well.\n"
    f"- You also have access to the screenshot of the current screen by \"Current Screen (image screenshot)\"; when text is unclear or the image provides more accurate or additional information, use it to inform your actions.\n"
    ...
    f"    1. The testimony is displayed in color=#00f000 in the \"Current Screen (image screenshot)\" and the \"Current State (text)\" includes **Cross-Examination!**\n"
    ...
    f"- Only the on-screen testimony (marked as [**The Conversation Currently on Screen**] in \"Current State (text)\" and the corresponding in \"Current Screen (image screenshot)\") can trigger the actions. Even if your analysis or long-term memory indicates a contradiction, continue pressing \"Ok\" until the corresponding testimony appears on the screen.\n"
