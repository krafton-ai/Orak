...
def parse_configs():
    ...
    parser.add_argument(
        "--config", type=str, default="configs/star_craft/play_full_game.yaml"
    )
...
async def main():
    ...
    score, step = await runner.mcp_play(
        config.game_server, config.agent_server, config.env.log_path, config
    )
